...
def _parse_since(since: str | None) -> datetime | None:
    ...
    try:
        ...
        raise HTTPException(
            400, f"invalid since timestamp: {since!r} (expected ISO-8601)"
        )


def _query(
    model, db: Session, run_id: UUID, since: datetime | None, limit: int
) -> list[dict[str, Any]]:
...
@router.get(
    "/{run_id}/sensor_readings",
    summary="Sensor readings for one run, optionally since a timestamp",
    response_model=list[dict[str, Any]],
)
def get_sensor_readings(
    run_id: str,
    since: str | None = Query(None, description="ISO-8601 lower bound on time"),
    limit: int = Query(
        DEFAULT_LIMIT,
        ge=1,
        le=MAX_LIMIT,
    ...
    """
    return _query(SensorReading, db, _parse_run_id(run_id), _parse_since(since), limit)


@router.get(
    "/{run_id}/actuator_states",
