...
class model2seek:
    def __init__(
        ...
        self.access_policy_: dict = {
            ...
            "manage": {
                "name": "manage",
                "description": "View, download, edit, and manage",
            },
        }
        ...
        self.license_: dict = {
            ...
            "CC-BY-SA-4.0": {
                "name": "CC-BY-SA-4.0",
                "description": "Creative Commons Attribution Share-Alike 4.0",
            },
            "ODC-BY-1.0": {
                "name": "ODC-BY-1.0",
            ...
            "ODbL-1.0": {
                "name": "ODbL-1.0",
                "description": "Open Data Commons Open Database License 1.0",
            },
            "ODC-PDDL-1.0": {
                "name": "ODC-PDDL-1.0",
                "description": "Open Data Commons Public Domain Dedication and Licence 1.0",
            },
            "notspecified": {
                "name": "notspecified",
                "description": "License Not Specified",
            },
            ...
            "GFDL-1.3-no-cover-texts-no-invariant-sections": {
                ...
                "description": "GNU Free Documentation License 1.3 with no cover texts and no invariant sections",
            },
            "geogratis": {"name": "geogratis", "description": "Geogratis"},
            "hesa-withrights": {
                "name": "hesa-withrights",
                "description": "Higher Education Statistics Agency Copyright with data.gov.uk rights",
            },
            "localauth-withrights": {
                "name": "localauth-withrights",
                "description": "Local Authority Copyright with data.gov.uk rights",
            },
            ...
            "OGL-Canada-2.0": {
                "name": "OGL-Canada-2.0",
                "description": "Open Government License 2.0 (Canada)",
            },
            "OSL-3.0": {"name": "OSL-3.0", "description": "Open Software License 3.0"},
            "dli-model-use": {
                "name": "dli-model-use",
                "description": "Statistics Canada: Data Liberation Initiative (DLI) - Model Data Use Licence",
            },
            "Talis": {"name": "Talis", "description": "Talis Community License"},
            "ukclickusepsi": {
                "name": "ukclickusepsi",
                "description": "UK Click Use PSI",
            },
            "ukcrown-withrights": {
                "name": "ukcrown-withrights",
                "description": "UK Crown Copyright with data.gov.uk rights",
            },
            "ukpsi": {
                "name": "ukpsi",
                "description": "UK PSI Public Sector Information",
            },
        }
        ...
        self.media_types_: dict = {
            ...
            "application/vnd.api+json": {
                "type_": "application/vnd.api+json",
                "for": "vnd.api+json",
            },
            "application/application/vnd.microsoft.portable-executable": {
                "type_": "application/application/vnd.microsoft.portable-executable",
                "for": [".efi", ".exe", ".dll"],
            },
            "application/vnd.ms-excel": {
                "type_": "application/vnd.ms-excel",
                "for": ".xls",
            },
            "application/vnd.ms-powerpoint": {
                "type_": "application/vnd.ms-powerpoint",
                "for": ".ppt",
            },
            "application/vnd.oasis.opendocument.text": {
                "type_": "application/vnd.oasis.opendocument.text",
                "for": ".odt",
            },
            ...
            "application/x-www-form-urlencoded": {
                "type_": "application/x-www-form-urlencoded",
                "for": None,
            },
            ...
            "application/x-python-code": {
                "type_": "application/x-python-code",
                "for": ".pyc",
            },
        }
    ...
    def check_model_vars(
        ...
        if len(model_creators) > 0 and isinstance(model_creators, list):
            for creator_ in model_creators:
                ...
                output_creators[creator_]["first_name"] = resource_["data"][
                    "attributes"
                ]["first_name"]
                output_creators[creator_]["last_name"] = resource_["data"][
                    "attributes"
                ]["last_name"]
                output_creators[creator_]["orcid"] = resource_["data"]["attributes"][
                    "orcid"
                ]
                output_creators[creator_]["name"] = (
                    output_creators[creator_]["first_name"]
                    + " "
                    + output_creators[creator_]["last_name"]
                )
                output_creators[creator_]["description"] = (
                    output_creators[creator_]["name"]
                    + " "
                    + output_creators[creator_]["orcid"]
                )

                output_simple.append(
                    "Creator: " + output_creators[creator_]["description"]
                )
        ...
        if len(model_organisms) > 0 and isinstance(model_organisms, list):
            for organism_ in model_organisms:
                ...
                output_organisms[organism_]["title"] = resource_["data"]["attributes"][
                    "title"
                ]
                output_organisms[organism_]["concept_uri"] = resource_["data"][
                    "attributes"
                ]["concept_uri"]
                output_organisms[organism_]["ontology_id"] = resource_["data"][
                    "attributes"
                ]["ontology_id"]

                output_organisms[organism_]["description"] = (
                    output_organisms[organism_]["title"]
                    + " "
                    + output_organisms[organism_]["concept_uri"]
                )

                output_simple.append(
                    "Organism: " + output_organisms[organism_]["description"]
                )
    ...
    def add_model(
        ...
        access_: str = "no_access",
        license_: str = "notspecified",
        model_type: str = "AI/ML",
        model_format: str = "Python code",
        ...
        if access_ not in list(self.access_policy_.keys()):
            raise ValueError(
                f"ERROR! access_ must be one of: {list(self.access_policy_.keys())} "
            )

        if license_ not in list(self.license_.keys()):
            raise ValueError(
                f"ERROR! license_ must be one of: {list(self.license_.keys())} "
            )

        if model_type not in self.SEEK_model_types_list:
            raise ValueError(
                f"ERROR! model_type must be one of: {self.SEEK_model_types_list} "
            )

        if model_format not in self.SEEK_model_formats_list:
            raise ValueError(
                f"ERROR! model_format must be one of: {self.SEEK_model_formats_list} "
            )

        if model_environment not in self.SEEK_model_recommended_envs_list:
            raise ValueError(
                f"ERROR! model_environment must be one of: {self.SEEK_model_recommended_envs_list} "
            )
        ...
        model["data"]["relationships"]["projects"]["data"] = [
            {"id": str(containing_project_id), "type": "projects"}
        ]
        ...
        if len(model_creators) > 0 and isinstance(model_creators, list):
            ...
            for person_ in model_creators:
                model["data"]["relationships"]["creators"]["data"].append(
                    {"id": person_, "type": "people"}
                )

        else:
            raise ValueError(
                "add_model: model_creators must be a list even if its a single item"
            )
        ...
        if len(model_organisms) > 0 and isinstance(model_organisms, list):
            ...
            for organism_ in model_organisms:
                model["data"]["relationships"]["creators"]["data"].append(
                    {"id": organism_, "type": "organisms"}
                )

        else:
            raise ValueError(
                "add_model: model_organisms must be a list even if its a single item"
            )
        ...
        with open(model_filepath, "rb") as file_:
            upload = self.session.put(
                blob_url, data=file_, headers={"Content-Type": "text/xml"}
            )
    ...
    def load_seek_terms(self, path_: str = ".\SEEK_yaml") -> None:
        ...
        """
        self.SEEK_model_types, _ = self.load_yaml(
            os.path.join(path_, "model_types.yml")
        )
        self.SEEK_model_formats, _ = self.load_yaml(
            os.path.join(path_, "model_formats.yml")
        )
        self.SEEK_model_recommended_envs, _ = self.load_yaml(
            os.path.join(path_, "model_recommended_environments.yml")
        )
        ...
        for key_ in self.SEEK_model_recommended_envs:
            self.SEEK_model_recommended_envs_list.append(
                self.SEEK_model_recommended_envs[key_]["title"]
            )
    ...
    def get_extended_metadata(
        ...
        r = requests.get(
            self.base_url + "/" + type_ + "/" + str(id_), headers=self.headers
        )
        r.raise_for_status()
        return r.json()
    ...
    def make_description(self, print_: bool = False) -> str:
        ...
        model_type_name = self.metadata["ml_model_configuration"][
            "model_identification"
        ]["name"]
        model_vers = self.metadata["ml_model_configuration"]["model_identification"][
            "version"
        ]
        ...
        # Model publication
        # -------------------------------
        model_publication_authors = self.metadata["ml_model_configuration"][
            "model_identification"
        ]["author"]
        model_publication_doi = self.metadata["ml_model_configuration"][
            "model_identification"
        ]["doi"]
        model_publication = {
        ...
        # Model description
        # -------------------------------
        model_file = self.metadata["ml_model_configuration"]["model_description"][
            "config_files"
        ]["model_file"]
        model_learner = self.metadata["ml_model_configuration"]["model_description"][
            "learner"
        ]
        model_type = self.metadata["ml_model_configuration"]["model_description"][
            "model_type"
        ]

        model_language = self.metadata["ml_model_configuration"]["model_description"][
            "language"
        ][0]["name"]
        model_language_vers = self.metadata["ml_model_configuration"][
            "model_description"
        ]["language"][1]["version"]

        model_time_int = self.metadata["ml_model_configuration"]["model_description"][
            "input_time_interval"
        ]["description"]
        model_aggregation = self.metadata["ml_model_configuration"][
            "model_description"
        ]["input_time_interval"]["aggregation"]["description"]
        if model_aggregation == "NaN":
            model_aggregation = 'No aggregation method is applied (set to "NaN").'

        model_package_requirements = ""
        for package in self.metadata["ml_model_configuration"]["model_description"][
            "packages"
        ]:
            model_package_requirements = (
                model_package_requirements + "* Package: " + package["package"] + "\n"
            )
            model_package_requirements = (
                model_package_requirements + "* Class: " + package["class"] + "\n"
            )
            model_package_requirements = (
                model_package_requirements + "* Version: " + package["version"] + "\n"
            )

        # -------------------------------
        # Model summary
        # -------------------------------
        model_description_ = self.metadata["ml_model_configuration"][
            "model_description"
        ]["description"]
        ...
        # Model training info
        # -------------------------------
        model_training_instance = str(
            self.metadata["ml_model_configuration"]["training_information"][
                "number_of_instances"
            ]
        )
        # model_training_hyper     = self.metadata['ml_model_configuration']['training_information']['hyperparameters']
        model_training_hyper = ""
        for hyper_param in self.metadata["ml_model_configuration"][
        ...
        model_training_valid = self.metadata["ml_model_configuration"][
            "training_information"
        ]["validation"]
        ...
        for input_ in self.metadata["ml_model_configuration"]["inputs"]["features"]:
            input_txt = (
                ...
                + str(input_["lag"])
                + " | "
                + input_["feature_scaling"]
                + " | "
                + str(input_["expected_range"]["min"])
                + " | "
                + str(input_["expected_range"]["max"])
                + "\n"
            )
        ...
        output_txt = ""
        for output_ in self.metadata["ml_model_configuration"]["outputs"][
            "information"
        ]:
            output_txt = (
                output_txt
                ...
                + output_["units"]
                + " | "
                + str(output_["forecast_horizon"])
                + " | "
                + output_["feature_scaling"]
                + " | "
                + str(output_["expected_range"]["min"])
                + " | "
                + str(output_["expected_range"]["max"])
                + "\n"
            )
