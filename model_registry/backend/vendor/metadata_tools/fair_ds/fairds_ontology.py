...
# -----------------------
def check_labels(
    term_fname: str = "terms.tsv",
    separator_: str = "\t",
    term_file_label_index: int = 0,
    metadata_fnames: list = [
        "Assay.tsv",
        "Investigation.tsv",
        "ObservationUnit.tsv",
        "Sample.tsv",
    ...
    metadata_file_label_index: int = 2,
) -> None:
    ...
    with open(term_fname) as term_file:
...
class fairds_ontology:
    def __init__(
        ...
        regex_sheet_headers: list = [
            "short hand form",
            "long form",
            "example name",
            "description",
        ],
        terms_sheet_headers: list = [
            "Item label",
            "Value syntax",
            "Example",
            "Preferred unit",
        ...
        fairds_ontology_xls_filename: str = "metadata.xlsx",
    ) -> None:
    ...
    def export_dict(self, dict_to_export: dict) -> None:
        ...
        # ----------------------
        # Make the json dump
        json.dump(dict_to_export, open("terms_dict.json", "w"), indent=4)
                    ...
                    if isinstance(dict_to_check[key_i][key__ii], dict):
                        for key___iii in dict_to_check[key_i][key__ii]:
                            if pd.isna(dict_to_check[key_i][key__ii][key___iii]):
                                dict_to_check[key_i][key__ii][key___iii] = (
                                    replace_nan_with
                                )
    ...
    def load_from_excel(self) -> None:
        ...
        with pd.ExcelFile(self.metadata_filepath) as xls:
            ...
            self.investigation = pd.read_excel(
                xls, "Investigation", names=self.level_sheet_headers
            )
            self.study = pd.read_excel(xls, "Study", names=self.level_sheet_headers)
            self.observationunit = pd.read_excel(
                xls, "ObservationUnit", names=self.level_sheet_headers
            )
    ...
    def convert_to_excel(self) -> None:
        ...
        dataframes = {
            os.path.splitext(os.path.basename(file))[0]: pd.read_csv(file, sep="\t")
    ...
    def convert_to_tsv(
        ...
        for sheet_name in wb.sheetnames:
            ...
            headers = [
                cell.value for cell in ws[header_row_idx]
            ...
            for row in ws.iter_rows(min_row=data_row_idx, values_only=False):
                ...
                for cell in row:
                    ...
                    if isinstance(cell.value, bool):
                        ...
                        )  # Or `row_data.append(str(cell.value))` for 'True'/'False'

                    elif cell.number_format in [
                        "0%",
                        "0.00%",
            ...
            if len(tmp_) > 0:
                for item in tmp_:
                    if item in self.terms_units_dict:
                        ...
                        self.terms_units_dict[item]["Item label"].append(
                            row["Item label"]
                        )
    ...
    def check_terms(self):
        ...
        # ------------------
        # Iterate over sheets
        for metadata_hierarchical_level in [
            self.investigation,
            self.study,
            self.observationunit,
            self.sample,
            ...
            for idx, row in metadata_hierarchical_level.iterrows():
                ...
                assert term in self.terms_all, (
                    f"term missing from term sheet, got: {term} at row {idx}"
                )

    def query_term_units(
        self, query_: str = "", print_values: bool = True, return_value: bool = False
    ):
            ...
            for idx, row in dataframe_item.iterrows():
                ...
                if found_key is not None:
                    if pd.isna(row["value syntax"]):
                        dataframe_item.loc[idx, "value syntax"] = found_key[
                            "Value syntax"
                        ]
                        print(
                            f"\t Changed {key_} value syntax to {found_key['Value syntax']}"
                        )
        ...
        for term_ in self.terms_dict.keys():
            ...
            if pd.isna(example_):
                ...
                if re.search(str(example_), syntax_to_test):
                    ...
                    print(
                        f"\t {term_}: example {example_} does not match syntax {syntax_to_test}"
                    )
                    not_matching += 1
