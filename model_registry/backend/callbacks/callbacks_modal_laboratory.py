...
def register_laboratory_modal_callbacks(app):
    ...
    def load_departments_dropdown(n, n_list, session_data):
        ...
        return [
            {"label": dept.name, "value": str(dept.id)} for dept in deps
        ], session_data
    ...
    def save_laboratory(n, name, location, dept_id, lab_id, session_data):
        ...
        if lab_id:
            ...
            _, session_data = service.update_laboratory(
                session_data, lab_id, name, location, dept_id
            )
        else:
            logger.debug("Creating new laboratory")
            _, session_data = service.create_laboratory(
                session_data, name, location, dept_id
            )

        logger.debug(
            f"Saved laboratory: {name}, location: {location}, dept_id: {dept_id}"
        )
    ...
    def open_edit_laboratory(n_clicks_list, session_data):
        ...
        service = LaboratoryService()
        (lab, dept_id), session_data = service.get_laboratory_with_dept(
            session_data, lab_id
        )
        if lab is None:
            raise PreventUpdate
        logger.debug(
            f"Editing laboratory with ID: {lab_id}, name: {lab.name}, dept_id: {dept_id}"
        )
    ...
    def update_labs(department_id, session_data):
        ...
        options = [{"label": lab.name, "value": str(lab.id)} for lab in labs]
