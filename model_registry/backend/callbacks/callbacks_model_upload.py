...
def _render_scaler_list(scalers):
    ...
    for s in scalers:
        rows.append(
            dbc.ListGroupItem(
                dbc.Row([
                    dbc.Col(html.Span(s["label"], className="fw-semibold"), width=8),
                    dbc.Col(html.Small(s["filename"], className="text-muted"), width=3),
                    dbc.Col(
                        dbc.Button(
                            "×",
    ...
    def update_output(contents, filename, model_info, session_data):
        ...
        if validation_error:
            return html.Div([
                ...
                ], color="danger", className="py-2 px-3 mt-2"),
            ]), model_info
        ...
        try:
            ...
            return html.Div([
                dbc.Alert([
                    ...
                    html.Span(f"  ·  {size_kb} KB  ·  .{extension}", className="text-muted ms-1"),
                ], color="success", className="py-2 px-3 mt-2"),
            ]), metadata
...
# ----- Callback to features -----
    ...
    def save_metadata(
        ...
        learner, algorithm_family, ml_task, model_category, model_type, model_name,
        description, language, language_version,
        ...
        f_names, f_types, f_units, f_lags, f_scalings, f_mins, f_maxs, f_descs, f_ids,
        o_names, o_units, o_horizons, o_scalings, o_mins, o_maxs, o_descs, o_ids,
        template_config,
        features_store, outputs_store, scalers_store,
        session_data,
    ):
        ...
        for i, _fid_obj in enumerate(f_ids or []):
            ...
            inputs.append({
                ...
                "scaler_id":       scaler_id,
                "scaler_filename": scaler_info.get("filename") or _fs.get("scaler_filename") or "",
                "scaler_path":     scaler_info.get("path")     or _fs.get("scaler_path")     or "",
        ...
        for i, _oid_obj in enumerate(o_ids or []):
            ...
            outputs.append({
                ...
                "scaler_id":       scaler_id,
                "scaler_filename": scaler_info.get("filename") or _os.get("scaler_filename") or "",
                "scaler_path":     scaler_info.get("path")     or _os.get("scaler_path")     or "",
                "has_scaler":      scaler_id is not None,
            })
        ...
        if template_config and isinstance(template_config, dict) and template_config.get("algorithm"):
        ...
        # PAYLOAD
        # =======================
        _experiments = [e.strip() for e in (experiments_id or "").split(",") if e.strip()]
        ...
        payload = {
            ...
            "input_time_interval": {
                ...
                "aggregation": {"method": agg_value or "", "description": agg_desc or ""},
        ...
        try:
            ...
            if result is None:
                return (
                    True,
                    dbc.Alert("❌ Error: could not save model to the database.", color="danger"),
                    "danger",
                    "Error",
                    dbc.Alert("❌ Error: could not save model to the database.", color="danger"),
                )
    ...
    def add_scaler_to_library(n_clicks, contents, filename, scaler_name, scalers, model_info):
        import uuid as _uuid, base64, os
        ...
        feedback = dbc.Alert(f"✅ Scaler '{label}' added.", color="success",
