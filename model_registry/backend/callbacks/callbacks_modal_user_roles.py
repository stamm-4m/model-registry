...
def register_user_modal_role_callbacks(app):
    ...
    def open_roles_modal(n_clicks_list, session_data):
        ...
        values = [
            str(r.role_id)
            for r in user_roles
            if getattr(r, "permission_id", None) is None
        ]
    ...
    def show_permissions(role_ids):
        ...
        return html.Ul([html.Li(p.description) for p in permissions])
    ...
    def load_model_roles(model_id, user_id, session_data):
        ...
        logger.debug(
            f"[callback] user_roles crudos: {[(ur.user_id, ur.role_id, ur.permission_id, ur.real_resource_id) for ur in user_roles]}"
        )

        permission_ids = [
            str(r.permission_id)
            for r in user_roles
            if getattr(r, "permission_id", None) is not None
            and str(getattr(r, "real_resource_id", None)) == str(model_id)
        ]
        ...
        logger.info(
            f"[load_model_roles] assigned_permission_ids: {list(assigned_permission_ids)}"
        )
    ...
    def save_roles_and_model_roles(
        n_clicks, role_ids, user_id, model_id, model_permission_ids, session_data
    ):
        if not n_clicks:
            raise PreventUpdate
        service = UserService()
        ...
        if model_id:
            ...
            service.assign_user_model_permissions(
                session_data,
                user_id,
                selected_role_ids,
                valid_model_permission_ids,
                model_id,
            )
