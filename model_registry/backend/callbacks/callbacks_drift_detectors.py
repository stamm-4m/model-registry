...
def _packs_table(rows):
    if not rows:
        return html.Div(
            "No packs yet. Use “Add pack” to upload a .zip package.",
            className="text-muted fst-italic p-3",
        )
    rows = sorted(
        rows, key=lambda r: (r.get("name", ""), r.get("version", "")), reverse=True
    )
    header = html.Thead(
        html.Tr(
            [
...
def _detectors_list(detectors):
    ...
    if not detectors:
        return html.Div(
            "This pack lists no detectors.", className="text-muted fst-italic"
        )
    detectors = sorted(
        detectors, key=lambda d: (d.get("kind", ""), d.get("detector_id", ""))
    )
    header = html.Thead(
        html.Tr([html.Th(c) for c in ["Detector", "Kind", "Description", "Parameters"]])
    )
    body = []
    for d in detectors:
        ...
        body.append(
            html.Tr(
                [
                    html.Td(
                        [
                            html.Strong(d.get("name") or d.get("detector_id")),
                            html.Br(),
...
def register_drift_detectors_callbacks(app):
    ...
    def submit_register(
        n, contents, filename, name, notes, activate, trig, session_data
    ):
        if not n:
            return (no_update,) * 6
        if not contents:
            return (
                dbc.Alert("Please choose a .zip file first.", color="warning"),
                True,
                no_update,
                no_update,
                no_update,
        ...
        try:
            ...
            return (
                dbc.Alert("Could not read the uploaded file.", color="danger"),
                True,
                no_update,
                no_update,
                no_update,
        ...
        if not result:
            return (
                dbc.Alert("Upload failed (no response from API).", color="danger"),
                True,
                no_update,
                no_update,
                no_update,
        ...
        if result.get("error"):
            return (
                dbc.Alert(f"Upload failed: {result['error']}", color="danger"),
                True,
                no_update,
                no_update,
                no_update,
        ...
        msg = (
            f"Added {pack.get('name')} v{pack.get('version')} — "
