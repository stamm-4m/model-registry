...
def register_department_modal_callbacks(app):
    ...
    def load_organizations_dropdown(n, n_list, session_data):
        ...
        return [{"label": org.name, "value": str(org.id)} for org in orgs], session_data
    ...
    def save_department(n, name, org_id, dept_id, session_data):
        ...
        if dept_id:
            ...
            _, session_data = service.update_department(
                session_data, dept_id, name, org_id
            )
        else:
    ...
    def open_edit_department(n_clicks_list, session_data):
        ...
        if dept is None:
            raise PreventUpdate
        org_id, session_data = service.get_organization_id_for_department(
            session_data, dept_id
        )
        logger.debug(
            f"Editing department with ID: {dept_id}, name: {dept.name}, org_id: {org_id}"
        )
        return (
            True,
            dept.name,
