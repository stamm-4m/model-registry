...
def build_temporal_saliency(features, seed_key):
    ...
    return _base_layout(
        fig, height=360, title="Temporal saliency |∂ŷ/∂xₜ| (feature × time)"
    )
...
def _panel(title, body, subtitle=None, tag=None, help=None):
    ...
    if help_text:
        ...
        header.append(
            dbc.Popover(
                dbc.PopoverBody(help_text, className="small"),
                target=hid,
                trigger="hover focus",
                placement="top",
            )
    ...
    if tag:
        header.append(
            dbc.Badge(
                tag, color="light", text_color="secondary", className="ms-2 border"
            )
        )
    return dbc.Card(
        dbc.CardBody(
            [
                ...
                html.Div(subtitle, className="text-muted small mb-2")
                if subtitle
                else None,
                body,
            ]
...
def _tree_family(profile, live, features, outputs, seed_key, family_label):
    ...
    if ok and live.get("importances"):
        ...
        imp_sub = (
            "Gini / variance reduction (placeholder — artifact not loadable here)."
        )

    tree_fig = (
        build_tree_fig_real(live["subtree"])
        if ok and live.get("subtree")
        else build_tree_fig(features, seed_key)
    )
    rules_body = (
        build_rule_list_real(live["rules_text"])
            ...
            dbc.Row(
                [
                    dbc.Col(
                        _panel(
                            "Feature interactions",
                            _graph(build_interaction_heatmap(features, seed_key)),
        ...
        return html.Div(
            [
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            _panel("Model coefficients", _graph(coef_fig), sub), lg=12
                        )
                    ],
                    className="g-3",
                ),
...
def _core_section(features, outputs, seed_key, supervised, live=None):
    ...
    if ok and perm:
        imp_fig = build_importance_fig_real(
        ...
        imp_tag = "placeholder"
    imp_panel = _panel(
        "Global feature importance", _graph(imp_fig), imp_sub, tag=imp_tag
    )
    ...
    row1 = dbc.Row(
        [
            ...
            dbc.Col(
                _panel("SHAP summary", _graph(shap_fig), shap_sub, tag=shap_tag), lg=6
            ),
        ],
        className="g-3",
    )
...
def model_explainability_layout(project_id=None, model_id=None, session_data=None):
    ...
    return html.Div(
        [
            ...
            html.Div(
                _core_section(features, outputs, seed_key, supervised, live),
