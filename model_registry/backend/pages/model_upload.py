...
def _pkg_rows(packages_jsonb, disabled=False):
    ...
    for i, pkg in enumerate(packages_jsonb or []):
        rows.append(
            package_row(
                i, package=pkg.get("package", ""), version=pkg.get("version", "")
            )
        )
    return rows
...
def _build_io_and_scalers(raw_inputs, raw_outputs):
    ...
    def _attach(items, filename, role):
        if not filename:
            return
        sid = next(
            (sc["id"] for sc in library.values() if sc["filename"] == filename), None
        )
        if sid is None:
    ...
    _attach(
        feat_items,
        raw_inputs.get("scaler") if isinstance(raw_inputs, dict) else None,
        "x",
    )
    _attach(
        out_items,
        raw_outputs.get("scaler") if isinstance(raw_outputs, dict) else None,
        "y",
    )
...
def model_form_layout(
    mode: str, project_id: str, model_id: str = None, session_data=None
):
    ...
    titles = {
        ...
        "details": (
            "Model Details",
            "Read-only view of model metadata and configuration.",
        ),
    }
                                            ...
                                            [
                                                ...
                                                dcc.Upload(
                                                    ...
                                                    children=html.Div(
                                                        [
                                                            html.I(
                                                                className="bi bi-cloud-upload fs-1 text-primary d-block mb-2"
                                                            ),
                                                            html.Span(
                                                                "Drag & drop your model file here, or "
                                                            ),
                                                            html.Span(
                                                                "browse",
                                                                style={
                                                            ...
                                                            html.Br(),
                                                            html.Small(
                                                                "Accepted: .pkl  .joblib  .keras  .h5  .rds  .yaml  .r",
                                                        ...
                                                        if mode == "add"
                                                            ...
                                                            html.I(
                                                                className="bi bi-file-earmark-binary fs-1 text-muted d-block mb-2"
                                                            ),
                                                            html.Span(
                                                                v(
                                                                    "artifact_path",
                                                                    "No artifact on record",
                                                                )
                                                                or "No artifact on record",
                                                                className="text-muted",
                                                            ),
                                                        ],
                                    ...
                                    dbc.Card(
                                        dbc.CardBody(
                                            [
                                                ...
                                                html.Div(
                                                    id="scalers-list-container",
                                                    children=[
                                                        html.Span(
                                                            "No scalers uploaded yet.",
                                                            ...
                                                            [
                                                                dcc.Upload(
                                                                    ...
                                                                    children=html.Div(
                                                                        [
                                                                            html.I(
                                                                                className="bi bi-cloud-upload me-2 text-primary"
                                                                            ),
                                                                            "Drop scaler file or ",
                                                                            html.Span(
                                                                                "browse",
