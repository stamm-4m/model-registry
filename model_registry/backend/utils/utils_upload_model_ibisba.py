...
def get_available_creators():
    try:
        seek_py = seek(
            base_url=settings.MODEL2SEEK_BASE_URL, token=settings.MODEL2SEEK_API_TOKEN
        )
        seek_py.start_session()

        people = seek_py.get_known_assets(seek_url_for_asset="people", store_info=True)
...
def get_available_projects_ibisba():
    try:
        ...
        projects = seek_py.get_known_assets(
            seek_url_for_asset="projects", store_info=True
        )
...
def get_available_organisms():
    try:
        ...
        people = seek_py.get_known_assets(
            seek_url_for_asset="organisms", store_info=True
        )
...
def get_information_model(project_id, model_id):
    ...
    try:
        ...
        logger.error(
            f"Error fetching model information for project {project_id} and model {model_id}: {e}"
        )
    return model_info
