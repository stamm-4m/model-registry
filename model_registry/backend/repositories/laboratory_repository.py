...
class LaboratoryRepository(BaseRepository):
    def get_with_department(self, lab_id):
        result = (
            self.db.query(Laboratory, DepartmentLaboratory.department_id)
            .join(
    ...
    def get_all(self):
        return (
            self.db.query(Laboratory, Department.name.label("department_name"))
            .join(
                DepartmentLaboratory,
    ...
    def get_by_id(self, laboratory_id):
        return self.db.query(Laboratory).filter(Laboratory.id == laboratory_id).first()
    ...
    def create(self, name, location, department_id):
        ...
        lab_dept = DepartmentLaboratory(
            laboratory_id=laboratory.id, department_id=department_id
        )
