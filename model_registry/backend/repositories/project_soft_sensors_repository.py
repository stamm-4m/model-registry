...
class ProjectSoftSensorsRepository(BaseRepository):
    def get_by_project(self, project_id):
        return self.db.query(ProjectSoftSensor).filter(ProjectSoftSensor.project_id == project_id).all()

    def get_by_soft_sensor(self, soft_sensor_id):
        return self.db.query(ProjectSoftSensor).filter(ProjectSoftSensor.soft_sensor_id == soft_sensor_id).all()
