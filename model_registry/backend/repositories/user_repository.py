...
class UserRepository(BaseRepository):
    def get_all(self):
        return (
            ...
            .outerjoin(
                ...
                DepartmentLaboratory.laboratory_id == Laboratory.id,
            )
    ...
    def count_user_roles(self, user_id):
        return self.db.query(UserRole).filter(UserRole.user_id == user_id).count()
    ...
    def get_dept_id_by_user_id(self, user_id):
        result = (
            ...
            .join(
                DepartmentLaboratory,
                DepartmentLaboratory.department_id == Department.id,
            )
    ...
    def get_all_roles_by_user_id(self, user_id):
        ...
        result = self.db.query(UserRole).filter(UserRole.user_id == user_id).all()
        return result
    ...
    def create_add_role_to_user(
        self, user_id, role_id, permission_id=None, real_resource_id=None
    ):
        user_role = UserRole(
            user_id=user_id,
            role_id=role_id,
    ...
    def assign_user_model_roles(self, user_id, role_ids, model_id):
        ...
        self.delete_roles_by_user(
            user_id, permission_id=None, real_resource_id=model_id
        )
        for role_id in role_ids:
    ...
    def assign_user_model_permissions(
        self, user_id, role_ids, permission_ids, model_id
    ):
        """
        ...
        logger.info(
            f"Asignando permisos sobre modelo {model_id} para user {user_id} con roles generales {role_ids}"
        )
        for perm_id in permission_ids:
            if role_ids:
                self.create_add_role_to_user(
                    user_id,
                    role_ids[0],
                    permission_id=perm_id,
                    real_resource_id=model_id,
                )
            else:
                logger.warning(
                    f"No se encontró role_id para permiso {perm_id} entre los roles generales del usuario"
                )
        self.db.commit()
        self.close()
